...
public class TestSource implements CfgSource<TestData, TestSourceConfig, TestSourceFetchConfig> {
    @Override
    public TestData fetch(TestSourceFetchConfig config) {
        return TestData.builder().value(config.getValue()).timestamp(System.currentTimeMillis()).build();
    }
